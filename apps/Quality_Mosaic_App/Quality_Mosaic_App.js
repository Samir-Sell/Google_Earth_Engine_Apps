...
// Generate line from polygon for visualization
function generate_repeat(){
  ...
  if (names.length == 2){
    ...
    var layer_to_remove2 = layers.get(1); // Grab the first layer
    Map.remove(layer_to_remove); 
    Map.remove(layer_to_remove2); 
  }
  else if (names.length == 1){
    layer_to_remove = layers.get(0);
    Map.remove(layer_to_remove)
  }
  // var length = names.length; // Grab the length of the list of layers
  // while (length > 1) { // If there is at least one layer, trigger this
  ...
  polygon = aoi
  var coords_for_vis = aoi.coordinates()
  var polygon_for_vis = ee.Geometry.LinearRing(coords_for_vis.flatten())
  print(polygon_for_vis)
  clear_geometry()
  Map.addLayer(polygon_for_vis, null, "test")
...
function remove_previous_layer() {
  ...
  layers.forEach(function(lay){ // For each layer
    var lay_name = lay.getName(); // Grab the name
    names.push(lay_name); // Append it to the names list
  // if ((names.slice(0,1) == "test") && (names.length < 1)) {
  //   print("FUNCTION CUT EARLY")
  //   return ""
  // }
  var length = names.length; // Grab the length of the list of layers
  if (length > 1) { // If there is at least one layer, trigger this
    var layer_to_remove = layers.get(1); // Grab the first layer
    Map.remove(layer_to_remove); // Remove the previously grabbed layer
    }
  });
 }
